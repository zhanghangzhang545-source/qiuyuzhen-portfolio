...
技术形态：零依赖静态网站（hash 路由单页应用），已接入 Supabase 真实云端（前台只读读取、后台完整维护：结构化字段 + 媒体上传/替换/排序/删除 + 发布/下架生命周期 + 逻辑删除）。
...
- **作品（插画 / 漫画 / 油画）**：标题、简介、性质（原创 / 同人）、标签、年份、阶段、排序；整作品删除（逻辑删除，底层原图保留备份）。
- **作品媒体**：上传 / 替换封面、上传 / 替换 / 排序 / 删除作品多图（原位替换，保留位置；删除为逻辑删除，底层原图保留备份）。
- **漫画内页**：新增 / 替换内页图片、调整内页顺序、删除内页（逻辑删除，底层原图保留备份）。
- **证书**：替换证书图片（隐藏证书替换后不会自行公开）。
- **关于页**：姓名、拼音、个人简介（bio）、创作方向、教育经历、工作 / 展览经历、技能、荣誉、联系方式（新增为本地空白行，填写并失焦才写入；未填写不入库）。
- **首页与作品库精选**：通过 `home_featured`（首页专题）与 `works_pick`（作品库默认页精选）两个**独立维度**控制哪些作品进入首页 / 作品库精选。
- **媒体发布生命周期**：上传的媒体先存为草稿，由管理员显式「发布」才对公开；可「下架 / 重新发布」。删除为逻辑删除（底层原图保留备份，可无损找回），操作均须二次确认。
